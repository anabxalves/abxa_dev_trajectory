...
// (QUESTÃO 3)
BMPData * bmp_write(char * filename, BMPFileHeader * file_header, BMPHeader * bmp_header, BMPData *bmp_data)
{
    FILE *fp;
    unsigned int sizeBMP = 0;
    ...
    sizeBMP = (bmp_header->biWidth) * (bmp_header->biHeight);
    fwrite(bmp_data, sizeof(BMPData), sizeBMP, fp);
